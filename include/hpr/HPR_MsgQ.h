...
#include "HPR_Types.h"
#ifdef OS_WINDOWS //去除linux下的编译,会有waring
 ...
 * @param nTimeOut (IN) 超时时间，单位毫秒(ms), 小于等于0表示死等(INFINITE),default is HPR_INFINITE
 * @return 0成功，-1失败
 * @sa HPR_MsgQReceiveEx
 */
HPR_DECLARE HPR_INT32 CALLBACK HPR_MsgQSendEx(HPR_HANDLE hMsgQId, HPR_MSG_BLOCK_T* pMsg, HPR_UINT32 nTimeOut);

/**
 * HPR_MsgQReceiveEx 从消息队列接收消息
 * @param hMsgQId (IN) 消息队列句柄
 * @param pMsg (OUT) 消息块,消息块最大长度8096字节
 * @param iTimeOut(IN) 超时时间，单位毫秒(ms), 小于等于0表示死等(INFINITE),default is HPR_INFINITE
 * @return 0成功，-1失败
 * @sa HPR_MsgQSendEx
 */
HPR_DECLARE HPR_INT32 CALLBACK HPR_MsgQReceiveEx(HPR_HANDLE hMsgQId, HPR_MSG_BLOCK_T* pMsg, HPR_UINT32 nTimeOut);

/**
 * HPR_MsgQGet_MsgCount 获取消息队列中的消息的个数
 * @param hMsgQId (IN) 消息队列句柄
 * @return nums msg if success ,HPR_ERROR if else
 * @sa HPR_MsgQSendEx
 */
HPR_DECLARE HPR_INT32 CALLBACK HPR_MsgQGet_MsgCount(HPR_HANDLE hMsgQId);
