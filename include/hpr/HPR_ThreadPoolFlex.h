...
*/
HPR_DECLARE HPR_HANDLE CALLBACK HPR_ThreadPoolFlex_Create
(
    HPR_UINT32 nMaxThreadNum, 
    HPR_UINT32 nIldeTime, 
    HPR_VOIDPTR(CALLBACK *fWorkRoutine)(HPR_VOIDPTR), 
    HPR_UINT32 nStackSize, 
    HPR_INT32 nPriority
);
...
typedef struct
{
	...
	HPR_INT32 thr_alive;
	HPR_INT32 thr_idle;
	HPR_INT32 idle_timeout;
	HPR_INT32 work_items;

}HPR_THREADPOOL_INFO;
/**
* HPR_ThreadPoolFlex_Info
* @param hHandle (IN) created by HPR_ThreadPoolFlex_Create
* @param pInfo (IN) parameter to store threadpool info.
* @return 0 success, -1 fail.
* @sa HPR_ThreadPoolFlex_Create(),HPR_ThreadPoolFlex_Destroy()
*/
HPR_DECLARE HPR_INT32 CALLBACK HPR_ThreadPoolFlex_Info(HPR_HANDLE hHandle, HPR_VOIDPTR pInfo);


/**
* HPR_ThreadPoolFlex_Del_item
* @param hHandle (IN) created by HPR_ThreadPoolFlex_Create
* @param cbf_threadpool_match_item (IN) 回调函数，判断工作队列中任务项是否为需要的
* @param usr_data (IN)  用户数据
* @param job_data (OUT) 工作队列中任务
* @return 0 success, -1 fail.
*/
HPR_DECLARE HPR_INT32 CALLBACK HPR_ThreadPoolFlex_Del_Item
(
    HPR_HANDLE hHandle, 
    HPR_INT32 (CALLBACK *cbf_threadpool_match_item)(void*, void*),
    HPR_VOIDPTR usr_data,
    HPR_VOIDPTR* job_data
);
