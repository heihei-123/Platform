...
{
    struct HPR_IPADDRESS* pNext; //向前指针
    struct HPR_IPADDR_STRING struIpAddrString;//IP地址信息
};
...
{
    struct HPR_ADAPTER_INFO* pNext;                        //向前指针
    ...
    struct HPR_IPADDRESS* pIpAddrList;                     //IPv4地址列表
    struct HPR_IPADDRESS* pGatawayList;                    //网关地址列表(POSIX无效)
    struct HPR_IPADDRESS* pDhcpServerList;                 //DHCP服务器列表(POSIX无效)
    struct HPR_IPADDRESS* pIpAddrv6List;                   //IPv6地址列表
};
...
HPR_DECLARE HPR_INT32 CALLBACK HPR_GetAdapterInfo(struct HPR_ADAPTER_INFO** pAdaterInfo);
HPR_DECLARE HPR_INT32 CALLBACK HPR_FreeAdapterInfo(struct HPR_ADAPTER_INFO* pAdaterInfo);
#endif

HPR_DECLARE struct hostent* CALLBACK HPR_GetAddressByName(const char* pName);
...
HPR_DECLARE HPR_UINT64 CALLBACK HPR_Hton64(HPR_UINT64 nHost64);

HPR_DECLARE HPR_UINT64 CALLBACK HPR_Ntoh64(HPR_UINT64 nNet64);
